export function splitModule(source){
 const lesson=source.search(/^## (?:Lesson|Lección) 1/m)
 const applied=source.search(/^## (?:What this means|Qué significa esto)/m)
 const close=source.search(/^## (?:Module close|Cierre del módulo)/m)
...
export function parseQuestions(source,id){
 return source.split(/^## (?:Question|Pregunta) /m).slice(1).map((block,index)=>{
  ...
  const fallback=(/Pregunta/i.test(source)?'Pregunta ':'Question ')+(index+1)
  return {id:`M${id}-Q${String(index+1).padStart(2,'0')}`,title:heading.replace(/^\d+\s*[—-]?\s*/,'')||fallback,q:prompt,a:answers,c:correctMatch?correctMatch[1].charCodeAt(0)-65:0,r:correctMatch?.[2]?.trim()||''}
 })
}
...
export function parseVisuals(source){
 return source.split(/^## Visual \d+ — /m).slice(1).map(block=>{
  ...
  const es=/Objetivo|Elementos|Idea clave|Interacción/.test(block)
  return {title,labels:labels.length>2?labels:(es?['Evidencia','Decisión','Acción','Revisión']:['Evidence','Decision','Action','Review'])}
 })
}
