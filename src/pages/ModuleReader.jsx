...
export default function ModuleReader(){
 const {id,view='opening'}=useParams(),{lang}=useLanguage(),es=lang==='es',content=getModuleContent(lang)[id],meta=getModules(lang).find(m=>m.id===id); const {progress,visit,completeModule}=useProgress()
 const state=meta?moduleState(id,progress):'LOCKED',index=viewOrder.indexOf(view),safeView=index<0?'opening':view
 useEffect(()=>{if(content&&state!=='LOCKED'&&!progress.visited[`${id}:${safeView}`])visit(`${id}:${safeView}`,`/module/${id}/${safeView}`)},[id,safeView,content,state,visit,progress.visited])
 ...
 const labels=es?{opening:'inicio',theory:'teoría',visuals:'visuales',scenario:'escenario',activity:'actividad',check:'comprobación',close:'cierre'}:Object.fromEntries(viewOrder.map(v=>[v,v]))
